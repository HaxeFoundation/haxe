...
package_doc:
	...
	haxe -lib hxtemplo -lib hxparse -lib hxargs -lib markdown -cp $$(haxelib path dox | head -n 1)/src -dce no --run dox.Dox -t $$(haxelib path dox | head -n 1)/themes -theme haxe_api -D website "http://haxe.org/" --title "Haxe API" -o $(PACKAGE_OUT_DIR)/$(PACKAGE_FILE_NAME)_doc.zip -D version "$$(haxe -version 2>&1)" -i $$(haxelib path dox | head -n 1)bin/xml -ex microsoft -ex javax -ex cs.internal -D source-path https://github.com/HaxeFoundation/haxe/blob/$(BRANCH)/std/
