...
@:coreApi abstract FilePath(NativeFilePath) {
		...
		var result = if(this == '') {
			...
			if(this.fastCodeAt(0) == '\\'.code) {
				...
				if(this.length > 2 && isSeparator(this.fastCodeAt(2))) {
					...
					try {
						var driveCwd = Path.realpath(this.charAt(0) + ':.');
	...
	public function name():Null<FilePath> {
		...
		return new FilePath(i < 0 ? s : s.substr(i + 1));
	}
	 ...
	 */
	static inline function trimSlashes(s:String):String {
		var slashes = SEPARATOR == '\\' ? '/\\' : '/';
		return Syntax.callField(s, "rstrip", slashes);
		// var i = s.length - 1;
		// while(i >= 0) {
		// 	switch s.fastCodeAt(i) {
		// 		case '/'.code:
