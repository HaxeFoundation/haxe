...
extern class GZipStream extends cs.system.io.Stream {
	/**
	 ...
	 * @param array The byte array to read the data into.
	 ...
	 * still be pending.
	 */
	function BeginRead(array:cs.NativeArray<cs.UInt8>, offset:Int, count:Int, asyncCallback:cs.system.AsyncCallback, asyncState:Dynamic):cs.system.IAsyncResult;
	/**
	 ...
	 * @param array The array used to store decompressed bytes.
	...
	/**
	 * Asynchronously reads a sequence of bytes from the current GZip stream into a
	 * byte array, advances the position within the GZip stream by the number of bytes
	 * read, and monitors cancellation requests.
	 * @param array The buffer to write the data into.
	 * @param offset The byte offset in  at which to begin writing data from the GZip
	 * stream.
	 * @param count The maximum number of bytes to read.
	 ...
	 * than the requested number, or it can be 0 (zero) if the end of the GZip stream
	 * has been reached.
	 */
	...
	/**
	 * Writes compressed bytes to the underlying GZip stream from the specified byte
	 * array.
	...
	/**
	 * Asynchronously writes compressed bytes to the underlying GZip stream from the
