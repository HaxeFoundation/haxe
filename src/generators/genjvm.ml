...
module AnnotationHandler = struct
	let convert_annotations meta =
		let parse_path e =
			let sl = try string_list_of_expr_path_raise e with Exit -> Error.str_typing_error "Field expression expected" (pos e) in
			let path = match sl with
				| s :: sl -> List.rev sl,s
				| _ -> Error.str_typing_error "Field expression expected" (pos e)
			in
			path
		in
		let rec parse_value e = match fst e with
			...
			| _ -> Error.str_typing_error "Expected value expression" (pos e)
		in
		let parse_value_pair e = match fst e with
			| EBinop(OpAssign,(EConst(Ident s),_),e1) ->
				s,parse_value e1
			| _ ->
				Error.str_typing_error "Assignment expression expected" (pos e)
		in
		let parse_expr e = match fst e with
			| ECall(e1,el) ->
				...
				path,values
			| _ ->
				Error.str_typing_error "Call expression expected" (pos e)
		in
...
= object(self)
	...
	method call ret tr e1 el =
			...
			begin match el with
				| [e1;{eexpr = TTypeExpr mt;epos = pe}] ->
					...
					let path = match jsignature_of_type gctx (type_of_module_type mt) with
						| TObject(path,_) -> path
						| _ -> Error.str_typing_error "Class expected" pe
					in
					code#instanceof path;
					Some TBool
				| _ -> Error.str_typing_error "Type expression expected" e1.epos
			end;
		...
		| TIdent "__array__" | TField(_,FStatic({cl_path = (["java"],"NativeArray")},{cf_name = "make"})) ->
			...
			| TInst({cl_path = (["java"],"NativeArray")},[t]) ->
				...
				Some (array_sig jsig)
			| _ ->
				Error.str_typing_error (Printf.sprintf "Bad __array__ type: %s" (s_type (print_context()) tr)) e1.epos;
			end
		...
		| TConst TSuper ->
			let c,cf = match field_info with
				| Some ({super_call_fields = hd :: tl} as info) ->
					...
					hd
				| _ ->
					Error.str_typing_error "Something went wrong" e1.epos
			in
	...
	method texpr' ret e =
		...
		| TNew(c,tl,el) ->
			...
			| None -> Error.str_typing_error "Could not find overload" e.epos
			| Some (c',cf,_) ->
				let f () =
					let tl,_ = self#call_arguments cf.cf_type el in
					tl
		...
		| TIdent _ ->
			Error.str_typing_error (s_expr_ast false "" (s_type (print_context())) e) e.epos;
