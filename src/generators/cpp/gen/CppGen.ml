...
let has_new_gc_references class_def =
   let is_gc_reference field = (should_implement_field field) && (is_data_member field) && not (type_cant_be_null field.cf_type) in
...
let rec has_gc_references class_def =
   ...
   | _ -> false ) || has_new_gc_references class_def
...
let default_value_string ctx value =
   ...
   | TConst (TFloat float_as_string) -> "((Float)" ^ (Texpr.replace_separators float_as_string "") ^ ")"
...
let can_inline_constructor baseCtx class_def super_deps constructor_deps =
   ...
   | Some { cf_expr= Some super_func } ->
         ...
         !is_simple && (
            ...
            let deps,_ = CppReferences.find_referenced_types_flags baseCtx (TClassDecl class_def) "new" super_deps constructor_deps false false true in
...
let implementations class_def =
   ...
   let cpp_interface_impl_name interface =
      "_hx_" ^ (join_class_path interface.cl_path "_" ) in
   let iterator impl =
...
let needed_interface_functions implemented_instance_fields native_implementations =
   let have = List.map (fun field -> (field.cf_name, ())) implemented_instance_fields |> List.to_seq |> Hashtbl.of_seq in
   let want = ref [] in
   ...
   let rec gen_with_injection injection expr new_line =
      ...
      | CppFunction(func,_) ->
         ...
         | FuncStatic(clazz,_,field) ->
              (match get_meta_string field.cf_meta Meta.Native with
              | Some n -> out n
              | None -> (out (cpp_class_name clazz); out ("::" ^ (cpp_member_name_of field) ^ "_dyn()")))
         | FuncExpression(expr) ->
              gen expr;
...
let generate_main ctx super_deps class_def =
   ...
   let main_expression =
      ...
      | _ -> die "" __LOC__ ) in
   CppReferences.find_referenced_types ctx (TClassDecl class_def) super_deps (Hashtbl.create 0) false false false |> ignore;
   let depend_referenced = CppReferences.find_referenced_types ctx (TClassDecl class_def) super_deps (Hashtbl.create 0) false true false in
...
let dynamic_functions class_def = List.fold_left (fun result field ->
...
let generate_class_files baseCtx super_deps constructor_deps class_def inScriptable =
   ...
   let (hx_implementations, native_implementations) = implementations class_def in
   
   let implemented = hash_keys hx_implementations in
   let implementsNative = (Hashtbl.length native_implementations) > 0 in
