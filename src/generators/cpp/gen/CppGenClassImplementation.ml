...
let generate_protocol_delegate ctx class_def output =
   ...
   let dump_delegate field =
      ...
      |  TFun(args,ret) ->
         ...
         let fieldName,argNames = match get_meta_string field.cf_meta Meta.ObjcProtocol with
         | Some nativeName ->
...
let generate baseCtx class_def =
   ...
   let all_referenced = CppReferences.find_referenced_types ctx (TClassDecl class_def) super_deps constructor_deps false false scriptable in
   ...
   let (haxe_implementations, native_implementations) = implementations class_def in
   ...
   if (not (has_class_flag class_def CInterface)) && not nativeGen then begin
      ...
      if not (has_class_flag class_def CAbstract) then begin
         ...
         let array_arg_list inList =
            ...
            let i = ref (0-1) in
            String.concat "," (List.map (fun _ -> incr i; "inArgs[" ^ (string_of_int !i) ^ "]"  ) inList) in
      ...
      if implements_haxe then begin
            ...
            List.iter (fun interface_name ->
               (try let interface = Hashtbl.find haxe_implementations interface_name in
   ...
   List.iter (gen_field ctx class_def class_name false) class_def.cl_ordered_fields;
   ...
   let inline_constructor = can_inline_constructor baseCtx class_def super_deps (create_constructor_dependencies common_ctx) in
   if (not (has_class_flag class_def CInterface)) && not nativeGen && not inline_constructor && not (has_class_flag class_def CAbstract) then
   ...
   let sMemberFields = match reflective_members with
      | [] -> "0 /* sMemberFields */"
      | _ -> 
   ...
   let generate_script_function isStatic field scriptName callName =
      ...
      | TFun (args,return_type) when not (is_data_member field) ->
         ...
         let ret = match cpp_type_of return_type with TCppScalar("bool") -> "b" | _ -> CppCppia.script_signature return_type false in
         if (ret<>"v") then output_cpp ("ctx->return" ^ (CppCppia.script_type return_type false) ^ "(");
         ...
         let dump_call cast =
            ...
            let (signature,_,_) = List.fold_left (fun (signature,sep,size) (_,opt,t) ->
               ...
               (signature ^ (CppCppia.script_signature t opt ), ",", (size^"+sizeof(" ^ (CppCppia.script_size_type t opt) ^ ")") ) ) (ret,"","sizeof(void*)")  args
            in
            output_cpp ")";
            signature
         in
   ...
   if (scriptable && not nativeGen) then begin
      ...
      let dump_script_field idx (field,f_args,return_t) =
         ...
         if newInteface then begin
            ...
            List.iter (fun (name,opt, t ) ->
               output_cpp ("\t\t__ctx->push" ^ (CppCppia.script_type t opt) ^ "(" ^ (keyword_remap name) ^ ");\n" );
            ) f_args;
            let interfaceSlot = string_of_int( -(cpp_get_interface_slot ctx name) ) in
            output_cpp ("\t\t" ^ ret ^ "__ctx->run" ^ (CppCppia.script_type return_t false) ^ "(__GetScriptVTable()[" ^ interfaceSlot ^ "]);\n" );
            output_cpp "\t}\n";
         end else begin
            ...
            output_cpp ("\t\t" ^ ret ^ "__ctx->run" ^ (CppCppia.script_type return_t false) ^ "(" ^ vtable ^ ");\n" );
            output_cpp ("\t}  else " ^ ret );
      ...
      let list_iteri func in_list =
         let idx = ref 0 in
         List.iter (fun elem -> func !idx elem; idx := !idx + 1 ) in_list in
