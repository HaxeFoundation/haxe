...
let generate_type com t =
	...
	let rec print_field stat f =
		...
		| Method m ->
			...
			let tparams = (match f.cf_params with [] -> "" | l -> "<" ^ String.concat "," (List.map hack_tp' l) ^ ">") in
	...
	| TClassDecl c ->
		...
		p "extern %s%s %s" finalmod (if (has_class_flag c CInterface) then "interface" else "class") (stype (TInst (c,List.map hack_tp c.cl_params)));
	...
	| TEnumDecl e ->
		print_meta e.e_meta;
		p "extern enum %s {\n" (stype (TEnum(e,List.map hack_tp e.e_params)));
		List.iter (fun n ->
	...
	| TTypeDecl t ->
		print_meta t.t_meta;
		p "typedef %s = " (stype (TType (t,List.map hack_tp t.t_params)));
		p "%s" (stype t.t_type);
		p "\n";
	| TAbstractDecl a ->
		print_meta a.a_meta;
		...
		p "abstract %s" (stype (TAbstract (a,List.map hack_tp a.a_params)));
