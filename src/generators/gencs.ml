...
let generate con =
		...
		*)
		...
		let rec ensure_local e explain =
			match e.eexpr with
				...
				| _ -> gen.gcon.error (located ("This function argument " ^ explain ^ " must be a local variable.") e.epos); e
		in
				...
				(match e.eexpr with
					...
					| TCall ({ eexpr = TIdent "__fixed__" }, [ e ] ) ->
						...
						let rec loop = function
							...
							| _ ->
								trace (debug_expr e);
								gen.gcon.error (located "Invalid 'fixed' keyword format" e.epos)
						in
						(match e.eexpr with
							| TBlock bl -> loop bl
							| _ ->
			...
			and gen_syntax meth args pos =
				...
				| "code", code :: args ->
					let code, code_pos =
						...
						| _ -> abort (located "The `code` argument for cs.Syntax.code must be a string constant" code.epos)
					in
					begin
				...
				| "plainCode", [code] ->
					let code =
						match code.eexpr with
						| TConst (TString s) -> s
						| _ -> abort (located "The `code` argument for cs.Syntax.plainCode must be a string constant" code.epos)
					in
					write w (String.concat "\n" (ExtString.String.nsplit code "\r\n"))
				| _ ->
					abort (located (Printf.sprintf "Unknown cs.Syntax method `%s` with %d arguments" meth (List.length args)) pos)
			and do_call w e el =
		...
		let rec gen_fpart_attrib w = function
			...
			| EField( ef, f, _ ), _ ->
				...
				write w f
			| _, p ->
				gen.gcon.error (located "Invalid expression inside @:meta metadata" p)
		in
		...
		let rec gen_spart w = function
			| EConst c, p -> (match c with
				...
				| String(s,_) ->
					...
					write w (escape s);
					write w "\""
				| _ -> gen.gcon.error (located "Invalid expression inside @:meta metadata" p))
			| EField( ef, f, _ ), _ ->
				gen_spart w ef;
				write w ".";
				write w f
			...
			match cl_params with
				| (_ :: _) when not (erase_generics && is_hxgeneric (TClassDecl cl)) ->
					...
					let combination_error c1 c2 =
						gen.gcon.error (located ("The " ^ (get_constraint c1) ^ " constraint cannot be combined with the " ^ (get_constraint c2) ^ " constraint.") cl.cl_pos) in
		...
		let rec gen_prop w is_static cl is_final (prop,t,get,set) =
			...
			let check cf = match cf with
				| Some ({ cf_overloads = o :: _ } as cf) ->
						gen.gcon.error (located "Property functions with more than one overload is currently unsupported" cf.cf_pos);
						gen.gcon.error (located "Property functions with more than one overload is currently unsupported" o.cf_pos)
				| _ -> ()
			in
			check get;
			check set;
			...
			(match cf.cf_kind with
				...
				| Method (MethDynamic) when Type.is_physical_field cf ->
					(if is_overload || List.exists (fun cf -> cf.cf_expr <> None) cf.cf_overloads then
						gen.gcon.error (located "Only normal (non-dynamic) methods can be overloaded" cf.cf_pos));
						...
						(match cf.cf_expr with
							| Some e ->
								if not (check_empty e) then
									gen.gcon.error (located "The body of a zero argument constructor of a struct should be empty" e.epos)
							| _ -> ());
						List.iter (fun cf ->
		...
		| _ ->
			...
			(if not (PMap.is_empty !pairs) then try
				...
				let idx_t, v_t = match follow get.cf_type with
					...
					| _ -> gen.gcon.error (located "The __get function must be a function with one argument. " get.cf_pos); die "" __LOC__
				in
					...
					| TFun([], ret) ->
						(match real_type ret with
							| TInst( { cl_path = ([], "String") }, []) ->
								...
								newline w
							| _ ->
								gen.gcon.error (located "A toString() function should return a String!" cf.cf_pos)
						)
					| _ -> ()
				)
			with | Not_found -> ());
					...
					| TFun([], ret) ->
						(match real_type ret with
							| TAbstract( { a_path = ([], "Void") }, []) ->
								...
								newline w
							| _ ->
								gen.gcon.error (located "A finalize() function should be Void->Void!" cf.cf_pos)
						)
					| _ -> ()
				)
			with | Not_found -> ());
		...
		let gen_class w cl is_first_type =
			if (is_first_type == false) then begin
				if Meta.has Meta.AssemblyStrict cl.cl_meta then
					gen.gcon.error (located "@:cs.assemblyStrict can only be used on the first class of a module" cl.cl_pos)
				else if Meta.has Meta.AssemblyMeta cl.cl_meta then
					gen.gcon.error (located "@:cs.assemblyMeta can only be used on the first class of a module" cl.cl_pos);
			end;
			...
			let should_close = match change_ns (TClassDecl cl) (fst (cl.cl_path)) with
				| [] ->
					...
					if Meta.has Meta.AssemblyStrict cl.cl_meta then
						gen.gcon.error (located "@:cs.assemblyStrict cannot be used on top level modules" cl.cl_pos)
					else if Meta.has Meta.AssemblyMeta cl.cl_meta then
						gen.gcon.error (located "@:cs.assemblyMeta cannot be used on top level modules" cl.cl_pos);
						...
						(if (Meta.has Meta.CsUsing cl.cl_meta) then
							match (Meta.get Meta.CsUsing cl.cl_meta) with
								| _,_,p when not !is_first_type ->
									gen.gcon.error (located "@:cs.using can only be used on the first type of a module" p)
								| _,[],p ->
									gen.gcon.error (located "One or several string constants expected" p)
								| _,e,_ ->
									(List.iter (fun e ->
										match e with
										| (EConst(String(s,_))),_ -> write w (Printf.sprintf "using %s;\n" s)
										| _,p -> gen.gcon.error (located "One or several string constants expected" p)
									) e);
									newline w
						);
		...
		if not erase_generics then HardNullableSynf.configure gen
			(fun e ->
				match e.eexpr, real_type e.etype with
					...
					| _ ->
						trace (debug_type e.etype); gen.gcon.error (located "This expression is not a Nullable expression" e.epos); die "" __LOC__
			)
			(fun v t has_value ->
		...
		with | Not_found ->
			gen.gcon.error (located "Fields 'fieldIds' and 'fields' were not found in class haxe.lang.FieldLookup" flookup_cl.cl_pos)
		);
	...
	with TypeNotFound path ->
		con.error (located ("Error. Module '" ^ (s_type_path path) ^ "' is required and was not included in build.") null_pos));
	debug_mode := false
