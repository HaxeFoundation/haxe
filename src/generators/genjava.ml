	...
	let configure gen runtime_cl =
		...
		let mk_dyn_box boxed_t expr =
			let name = match boxed_t with
				...
				| TInst({ cl_path = (["java";"lang"],"Short") },[]) ->
					"numToShort"
				| _ -> gen.gcon.error (located ("Invalid boxed type " ^ (debug_type boxed_t)) expr.epos); die "" __LOC__
			in
			{
				eexpr = TCall(
...
let generate con =
	...
	*)
	...
	let add_import pos path meta =
		...
		let rec loop = function
			| (pack, n) :: _ when name = n ->
					if path <> (pack,n) then
						gen.gcon.error (located ("This expression cannot be generated because " ^ path_s path meta ^ " is shadowed by the current scope and ") pos)
			| _ :: tl ->
					loop tl
			| [] ->
					(* add import *)
	...
	let path_s_import pos path meta = match path with
		| [], name when PMap.mem name !scope ->
				gen.gcon.error (located ("This expression cannot be generated because " ^ name ^ " is shadowed by the current scope") pos);
				name
		| pack1 :: _, name when PMap.mem pack1 !scope -> (* exists in scope *)
				...
				if PMap.mem name !scope then
					gen.gcon.error (located ("This expression cannot be generated because " ^ pack1 ^ " and " ^ name ^ " are both shadowed by the current scope") pos);
				name
		| _ -> path_s path meta
	in
	...
	*)
						...
						(match cf.cf_type with
							| TFun([], ret) ->
								(match real_type ret with
									...
									| _ -> gen.gcon.error (located "A toString() function should return a String!" cf.cf_pos); false
								)
							| _ -> has_class_field_flag cf CfOverride
						)
						...
						(match cf.cf_type with
							| TFun([], ret) ->
								(match real_type ret with
									| TAbstract ({ a_path = ([], "Int") },[]) ->
										true
									| _ -> gen.gcon.error (located "A hashCode() function should return an Int!" cf.cf_pos); false
								)
							| _ -> has_class_field_flag cf CfOverride
						)
					| _ -> has_class_field_flag cf CfOverride
	...
	with TypeNotFound path -> con.error (located ("Error. Module '" ^ (s_type_path path) ^ "' is required and was not included in build.") null_pos));
	debug_mode := false
