...
let generate_source ctx =
   ...
   | Some e ->
	  ...
	  let class_structure = { (class_structure()) with cl_ordered_statics = [main_field] } in
      let class_def = { null_class with cl_path = ([],"@Main"); cl_structure = (fun () -> class_structure) } in
