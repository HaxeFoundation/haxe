		...
		*)
		method write_expr_call_syntax_extern expr args =
			...
			match name with
				...
				| "keepVar" -> ()
				| _ -> ctx.pgc_common.error_msg ("php.Syntax." ^ name ^ "() is not supported.") self#pos
		(**
		...
		method write_expr_syntax_code args =
			match args with
				...
				| { eexpr = TConst (TString php) } :: args ->
					...
					in
					Codegen.interpolate_code ctx.pgc_common php args self#write self#write_expr self#pos
				| _ -> ctx.pgc_common.error_msg "First argument of php.Syntax.code() must be a constant string." self#pos
		(**
		...
		*)
		method write_expr_syntax_assoc_decl args =
			match args with
				...
				| _ -> ctx.pgc_common.error_msg "php.Syntax.assocDecl() accepts object declaration only." self#pos
		(**
							...
							if m == PhpAttribute then begin
								...
								(match el with
									| [EConst (String (s,_)),p] ->
										writer#write s
									| _ ->
										ctx.pgc_common.error_msg ("@:php.attribute meta expects a single string constant as an argument.") p
								);
								writer#write "]\n";
								true
							end else
		...
		*)
		method private validate_method_name field =
			...
			if List.exists (fun n -> n = uppercased_name) used_method_names then
				ctx.pgc_common.error_msg ("Methods names are case-insensitive in PHP runtime. Cannot redeclare \"" ^ field.cf_name ^ "\".") field.cf_name_pos
			else
