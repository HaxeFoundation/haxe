...
let inline_config cls_opt cf call_args return_type =
	...
	| Some ({cl_kind = KAbstractImpl _}) when has_class_field_flag cf CfImpl ->
		let t = if cf.cf_name = "_new" then
			...
			str_typing_error "Invalid abstract implementation function" cf.cf_pos
		else
		...
		*)
		...
		let rec check_write e =
			...
			| TLocal v when has_var_flag v VFinal ->
				str_typing_error "Cannot modify abstract value of final local" p
			| TField(_,fa) ->
				begin match extract_field fa with
				| Some cf when has_class_field_flag cf CfFinal ->
					str_typing_error "Cannot modify abstract value of final field" p
				| _ ->
					()
				end
			...
			| TCast(e1,None) ->
				check_write e1
			| _  ->
				str_typing_error "Cannot modify the abstract value, store it into a local first" p;
		in
		let vars = List.fold_left (fun acc (i,e) ->
			...
			if i.i_abstract_this && i.i_write then begin
				...
				let vik = match e.eexpr with
					...
					| TFunction _ ->
						if i.i_write then str_typing_error "Cannot modify a closure parameter inside inline method" p;
...
let rec type_inline ctx cf f ethis params tret config p ?(self_calling_closure=false) force =
	...
	let rec map term in_call e =
		...
		| TConst TThis ->
			...
			| None ->
				str_typing_error "Could not inline `this` outside of an instance context" po
			)
		| TVar (v,eo) ->
		...
		| TReturn eo when not state#in_local_fun ->
			if not term then begin
				...
				| Method MethInline ->
					str_typing_error "Cannot inline a not final return" po
				| _ ->
					str_typing_error ("Function " ^ cf.cf_name ^ " cannot be inlined because of a not final return") p
			end;
			(match eo with
		...
		| TCall({eexpr = TConst TSuper; etype = t},el) ->
			...
			| TInst({ cl_constructor = Some ({cf_kind = Method MethInline; cf_expr = Some ({eexpr = TFunction tf})} as cf)} as c,_) ->
				...
				| None -> str_typing_error "Could not inline super constructor call" po
				end
			| _ -> str_typing_error "Cannot inline function containing super" po
			end
		| TCall(e1,el) ->
			state#set_side_effect;
			...
			{e with eexpr = TCall(e1,el)}
		| TConst TSuper ->
			str_typing_error "Cannot inline function containing super" po
