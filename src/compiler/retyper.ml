...
let pair_class_field rctx ctx cctx fctx cf cff p =
	...
	| FFun fd ->
		...
		let is_coroutine = Meta.has Meta.Coroutine cff.cff_meta in
		let t = TFun(args#for_type,ret,is_coroutine) in
		(fun () ->
			(* This is the only part that should actually modify anything. *)
			cf.cf_type <- t;
			TypeBinding.bind_method ctx cctx fctx cf t args ret fd.f_expr (match fd.f_expr with Some e -> snd e | None -> cff.cff_pos) is_coroutine;
