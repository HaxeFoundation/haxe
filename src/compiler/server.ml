		...
		let get_formatter def default =
			...
			match format_mode with
				...
				| m -> begin
					...
					error ctx (located (Printf.sprintf "Invalid message reporting mode: \"%s\", expected classic | pretty | indent (for -D %s)." m def) null_pos);
		...
		if !log_messages then begin
			try begin
				...
				| Failure e | Sys_error e -> begin
					...
					error ctx (located (Printf.sprintf "Error opening log file: %s. Logging to file disabled (-D %s)" e def) null_pos);
					log_messages := false;
				end
		end;
