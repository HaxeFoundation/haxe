...
let parse_args com =
	...
	let basic_args_spec = [
		...
		("Compilation",["-p";"--class-path"],["-cp"],Arg.String (fun path ->
			com.class_path#add (new ClassPath.directory_class_path (Path.add_trailing_slash path) User);
		),"<path>","add a directory to find source files");
		("Compilation",[],["-libcp"],Arg.String (fun path ->
			com.class_path#add (new ClassPath.directory_class_path (Path.add_trailing_slash path) Lib);
		),"<path>","add a directory to find source files");
