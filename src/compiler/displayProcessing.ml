...
let load_display_module_in_macro tctx display_file_dot_path clear = match display_file_dot_path with
	| Some cpath ->
		...
		begin try
			...
			let mctx = MacroContext.get_macro_context tctx p in
			...
			let _ = MacroContext.load_macro_module (MacroContext.make_macro_api tctx p) (MacroContext.get_macro_context tctx p) tctx.com cpath true p in
