...
let rec compare a b =
	...
	| VLazy f1,_ ->
		compare (AtomicLazy.force f1) b
	| _,VLazy f2 ->
		compare a (AtomicLazy.force f2)
	| _ -> CUndef
...
and equals_structurally a b =
	...
	| VLazy f1,_ -> equals_structurally (AtomicLazy.force f1) b
	| _,VLazy f2 -> equals_structurally a (AtomicLazy.force f2)
	| _ -> a == b
