			...
			let msg =
				...
				| HaxeError.Error err ->
						...
						HaxeError.recurse_error (fun depth err ->
							let cm = Message.make ~from_macro:err.err_from_macro (HaxeError.error_msg err.err_message) err.err_pos depth MKError in
							let ectx = MessageReporting.create_error_context false in
							match MessageReporting.message_string ectx cm with
								| None -> ()
