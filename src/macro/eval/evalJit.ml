...
and jit_expr jit return e =
	...
	| TFunction tf ->
		...
		let captures = ExtList.List.filter_map (fun (i,vid,declared) ->
			...
			else Some (i,fst (try Hashtbl.find jit.captures vid with Not_found -> Error.str_typing_error "Something went wrong" e.epos))
		) captures in
	...
	| TParenthesis e1 | TMeta(_,e1) | TCast(e1,None) ->
		loop e1
	| TIdent s ->
		Error.str_typing_error ("Unknown identifier: " ^ s) e.epos
	in
	let f = loop e in
