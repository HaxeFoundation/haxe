...
let rec is_nullable ?(no_lazy=false) = function
	| TMono r ->
		(match r.tm_type with None -> List.exists (function MNullable _ -> true | _ -> false) r.tm_modifiers | Some t -> is_nullable ~no_lazy t)
...
let rec is_null ?(no_lazy=false) = function
	| TMono r ->
		(match r.tm_type with None -> List.exists (function MNullable _ -> true |_ -> false) r.tm_modifiers | Some t -> is_null ~no_lazy t)
	| TAbstract ({ a_path = ([],"Null") },[t]) ->
...
let rec is_explicit_null = function
	| TMono r ->
		(match r.tm_type with None -> List.exists (function MNullable _ -> true | _ -> false) r.tm_modifiers | Some t -> is_explicit_null t)
	| TAbstract ({ a_path = ([],"Null") },[t]) ->
		true
	| TLazy f ->
