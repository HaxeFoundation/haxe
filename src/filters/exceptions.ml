...
let haxe_exception_static_call ctx method_name args p =
	let method_field =
		...
		with Not_found -> str_typing_error ("haxe.Exception has no field " ^ method_name) p
	in
	let return_type =
		match follow method_field.cf_type with
		| TFun(_,t) -> t
		| _ -> str_typing_error ("haxe.Exception." ^ method_name ^ " is not a function and cannot be called") p
	in
...
let haxe_exception_instance_call ctx haxe_exception method_name args p =
	...
	| FInstance (_,_,cf) as faccess ->
		...
		let rt =
			...
			| TFun(_,t) -> t
			| _ ->
				str_typing_error ((s_type (print_context()) haxe_exception.etype) ^ "." ^ method_name ^ " is not a function and cannot be called") p
		in
		make_call ctx.typer efield args rt p
	| _ -> str_typing_error ((s_type (print_context()) haxe_exception.etype) ^ "." ^ method_name ^ " is expected to be an instance method") p
...
let std_is ctx e t p =
	...
	let std_cls =
		...
		| TClassDecl cls -> cls
		| _ -> str_typing_error "Std is expected to be a class" p
	in
	let isOfType_field =
		try PMap.find "isOfType" std_cls.cl_statics
		with Not_found -> str_typing_error ("Std has no field isOfType") p
	in
	let return_type =
		match follow isOfType_field.cf_type with
		| TFun(_,t) -> t
		| _ -> str_typing_error ("Std.isOfType is not a function and cannot be called") p
	in
...
let filter tctx =
	...
	| Php | Js | Java | Cs | Python | Lua | Eval | Neko | Flash | Hl | Cpp ->
		...
		and haxe_exception_type, haxe_exception_class =
			...
			| _ -> str_typing_error "haxe.Exception is expected to be a class" null_pos
		and value_exception_type, value_exception_class =
			match Typeload.load_instance tctx (tp value_exception_type_path) true with
			| TInst(cls,_) as t -> t,cls
			| _ -> str_typing_error "haxe.ValueException is expected to be a class" null_pos
		and haxe_native_stack_trace =
			...
			| _ -> str_typing_error "haxe.NativeStackTrace is expected to be a class or an abstract" null_pos
		in
		...
		let save_exception_stack catch_var =
			...
			if has_feature tctx.com "haxe.NativeStackTrace.exceptionStack" then
				let method_field =
					...
					with Not_found -> str_typing_error ("haxe.NativeStackTrace has no field saveStack") null_pos
				in
				let return_type =
					match follow method_field.cf_type with
					| TFun(_,t) -> t
					| _ -> str_typing_error ("haxe.NativeStackTrace." ^ method_field.cf_name ^ " is not a function and cannot be called") null_pos
				in
				let catch_local = mk (TLocal catch_var) catch_var.v_type catch_var.v_pos in
				begin
			...
			| TClassDecl cls when not (has_class_flag cls CExtern) && cls.cl_path <> haxe_exception_type_path && is_haxe_exception_class cls ->
				let shift_stack p =
					...
					let faccess =
						...
						with Not_found -> str_typing_error "haxe.Exception has no field __shiftStack" p
					in
					match faccess with
					| FInstance (_,_,cf) ->
						...
						let rt =
							...
							| TFun(_,t) -> t
							| _ ->
								str_typing_error "haxe.Exception.__shiftStack is not a function and cannot be called" cf.cf_name_pos
						in
						make_call tctx efield [] rt p
					| _ -> str_typing_error "haxe.Exception.__shiftStack is expected to be an instance method" p
				in
