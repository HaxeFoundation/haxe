	...
	let default_implementation gen =
		...
		let rec run e =
			...
			| TReturn (eopt) ->
				...
				let ret_type = match !current_ret_type with | Some(s) -> s | None -> gen.gcon.error (located "Invalid return outside function declaration." e.epos); die "" __LOC__ in
				(match eopt with
...
let handle_type_parameter gen e e1 ef ~clean_ef ~overloads_cast_to_base f elist calls_parameters_explicitly =
	...
	| FEnumField (en, efield, true) ->
		let ecall = match e with | None -> trace (field_name f); trace efield.ef_name; gen.gcon.error (located "This field should be called immediately" ef.epos); die "" __LOC__ | Some ecall -> ecall in
