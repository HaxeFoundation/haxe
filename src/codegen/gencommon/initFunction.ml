...
let handle_class com cl =
	...
	let vars, funs = List.fold_left (fun (acc_vars,acc_funs) cf ->
		...
		| Method MethDynamic ->
			...
			| Some e, [] ->
				let var = mk (TField ((mk (TConst TThis) (TInst (cl, List.map hack_tp cl.cl_params)) cf.cf_pos), FInstance(cl, List.map hack_tp cl.cl_params, cf))) cf.cf_type cf.cf_pos in
				let ret = binop Ast.OpAssign var e cf.cf_type cf.cf_pos in
				cf.cf_expr <- None;
	...
	| _ ->
		...
		let ctors =
			...
			| None ->
				try
					let sctor, sup, stl = OverloadingConstructor.prev_ctor cl (List.map hack_tp cl.cl_params) in
