...
let save_types types =
	let fields_storage = ref []
	and vars_storage = ref []
	and class_storage = ref []
	in
	let save_var v =
		vars_storage := (v, v.v_type) :: !vars_storage
	in
	let rec traverse_expr e =
		match e.eexpr with
			| TFunction fn ->
				List.iter (fun (v, _) -> save_var v) fn.tf_args;
				traverse_expr fn.tf_expr
			| TVar (v, e) ->
				save_var v;
				Option.may traverse_expr e
			| _ ->
				iter traverse_expr e
	in
	let rec save_field cf =
		Option.may traverse_expr cf.cf_expr;
		fields_storage := (cf, cf.cf_expr, cf.cf_type) :: !fields_storage;
		List.iter save_field cf.cf_overloads;
	in
	let rec traverse_types types : unit =
		match types with
			| [] -> ()
			| TClassDecl cls :: rest ->
				List.iter save_field cls.cl_ordered_statics;
				List.iter save_field cls.cl_ordered_fields;
				Option.may save_field cls.cl_constructor;
				Option.may traverse_expr cls.cl_init;
				class_storage := (cls, cls.cl_dynamic, cls.cl_array_access, cls.cl_init, cls.cl_super, cls.cl_implements) :: !class_storage;
				traverse_types rest
			| _ :: rest ->
				traverse_types rest
	in
	...
	(fun () ->
		List.iter
			(fun (cls, cl_dynamic, cl_array_access, cl_init, cl_super, cl_implements) ->
				cls.cl_dynamic <- cl_dynamic;
				cls.cl_array_access <- cl_array_access;
				cls.cl_init <- cl_init;
				cls.cl_super <- cl_super;
				cls.cl_implements <- cl_implements;
			)
			!class_storage;
		List.iter
			(fun (cf, cf_expr, cf_type) ->
				cf.cf_expr <- cf_expr;
				cf.cf_type <- cf_type;
			)
			!fields_storage;
		List.iter (fun (v, t) -> v.v_type <- t) !vars_storage;
	)