...
let replace_super_call com c tl with_params me p follow_type =
	...
	let cf =
		...
		with Not_found ->
			com.error (Globals.located "No suitable overload for the super call arguments was found" p); cf
	in
	{
		eexpr = TCall(
			{
		...
		let rec map_expr ~is_first e = match e.eexpr with
			...
			with | Not_found ->
				...
				if not is_first then
					com.error (Globals.located "Super call must be the first call when extending native types" e.epos);
				{ e with eexpr = TBlock([]) })
...
let ensure_super_is_first com cf =
	let rec loop e =
		...
		| _ ->
			com.error (Globals.located "Types that derive from a native class must have its super() call as the first statement in the constructor" cf.cf_pos)
	in
