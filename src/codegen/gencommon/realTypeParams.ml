...
let rec follow_all_md md =
	let t = match md with
		| TClassDecl { cl_kind = KAbstractImpl a } ->
			TAbstract(a, List.map hack_tp a.a_params)
		| TClassDecl c ->
			TInst(c, List.map hack_tp c.cl_params)
		| TEnumDecl e ->
			TEnum(e, List.map hack_tp e.e_params)
		| TTypeDecl t ->
			TType(t, List.map hack_tp t.t_params)
		| TAbstractDecl a ->
			TAbstract(a, List.map hack_tp a.a_params)
	in
...
let set_hxgeneric gen md =
	try
		...
		if not ret then begin
			match md with
			| TClassDecl c ->
				let set_hxgeneric (_,param,_) = match follow param with
					| TInst(c,_) ->
...
struct
	...
	let create_stub_casts gen cl cast_cfield =
		...
		let p = cl.cl_pos in
		let this = { eexpr = TConst TThis; etype = (TInst(cl, List.map hack_tp cl.cl_params)); epos = p } in
		...
		let rec loop curcls params level reverse_params =
			if (level <> 0 || (has_class_flag curcls CInterface) || (has_class_flag curcls CAbstract) ) && params <> [] && is_hxgeneric (TClassDecl curcls) then begin
				let cparams = List.map (fun (s,t,d) -> (s, TInst (map_param (get_cl_t t), []),d)) curcls.cl_params in
				...
				if not (PMap.mem name cl.cl_fields) then begin
					...
					let field = { eexpr = TField(this, FInstance(cl,List.map hack_tp cl.cl_params, cast_cfield)); etype = apply_params cast_cfield.cf_params reverse_params cast_cfield.cf_type; epos = p } in
					let call =
					{
						eexpr = TCall(field, []);
						etype = t_dynamic;
		...
		in
		loop cl (List.map hack_tp cl.cl_params) 0 (List.map hack_tp cl.cl_params)
	...
	*)
	let create_cast_cfield gen cl name =
		...
		let cparams = List.map (fun (s,t,def) -> (s, TInst (map_param (get_cl_t t), []), def)) cl.cl_params in
		let cfield = mk_class_field name (TFun([], t_dynamic)) false cl.cl_pos (Method MethNormal) cparams in
		let params = List.map hack_tp cparams in

		let fields = get_fields gen cl (List.map hack_tp cl.cl_params) params [] in
		...
		*)
		...
		let fields_to_cases fields =
			...
			List.map (fun (cf, t_cl, t_cf) ->
				...
				let this_field = mk (TField (this, FInstance (cl, List.map hack_tp cl.cl_params, cf))) t_cl pos in
				let expr =
					binop
						OpAssign
						(mk (TField (local_new_me, FInstance(cl, List.map hack_tp cl.cl_params, cf))) t_cf pos)
		...
		let fn = {
			...
			tf_expr = mk (TBlock [
				(* if (typeof(T) == typeof(T2)) return this *)
				mk (TIf (mk_typehandle_cond (List.map hack_tp cl.cl_params) params, mk_return this, None)) basic.tvoid pos;
	...
	let create_static_cast_cf gen iface cf =
		...
		let basic = gen.gcon.basic in
		let cparams = List.map (fun (s,t,def) -> ("To_" ^ s, TInst (map_param (get_cl_t t), []),def)) cf.cf_params in
		...
		let field = { eexpr = TField(mk_local me p, FInstance(iface, List.map hack_tp iface.cl_params, cf)); etype = apply_params cf.cf_params params cf.cf_type; epos = p } in
		let call =
		{
			eexpr = TCall(field, []);
			etype = t_dynamic;
	...
	let default_implementation gen ifaces base_generic =
		...
		let implement_stub_cast cthis iface tl =
			...
			if not (PMap.mem name cthis.cl_fields) then begin
				let cparams = List.map (fun (s,t,def) -> ("To_" ^ s, TInst(map_param (get_cl_t t), []),def)) iface.cl_params in
				let field = mk_class_field name (TFun([],t_dynamic)) false iface.cl_pos (Method MethNormal) cparams in
				let this = { eexpr = TConst TThis; etype = TInst(cthis, List.map hack_tp cthis.cl_params); epos = cthis.cl_pos } in
				field.cf_expr <- Some {
