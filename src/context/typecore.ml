...
let type_expr_ref : (typer -> expr -> access_mode -> WithType.t -> texpr) ref = ref (fun _ _ _ _ -> assert false)
...
let type_expr ctx e mode with_type = (!type_expr_ref) ctx e mode with_type
