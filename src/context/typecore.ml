...
let display_identifier_error ctx ?prepend_msg msg p =
	...
	display_str_error ctx.com (prepend ^ msg) p
...
let check_module_path ctx (pack,name) p =
	...
	with Failure msg ->
		display_str_error ctx.com ("\"" ^ (StringHelper.s_escape (String.concat "." pack)) ^ "\" is not a valid package name:") p;
...
let check_field_access ctx c f stat p =
	if not ctx.untyped && not (can_access ctx c f stat) then
		display_str_error ctx.com ("Cannot access private field " ^ f.cf_name) p
	...
	let rec loop e pack name = match e with
		| EField(e,f,_),p when Char.lowercase (String.get f 0) <> String.get f 0 -> (match name with
			...
			| _ -> (* too many name paths *)
				display_str_error com ("Unexpected " ^ f) p;
				raise Exit)
		| EField(e,f,_),_ ->
			loop e (f :: pack) name
		| EConst(Ident f),_ ->
			let pack, name, sub = match name with
				| [] ->
					...
					if Char.uppercase fchar = fchar then
						...
						display_str_error com "A class name must start with an uppercase letter" (snd e);
						raise Exit
					end
				| [name] ->
					f :: pack, name, None
		...
		| _,pos ->
			display_str_error com "Unexpected expression when building strict meta" pos;
			raise Exit
	in
	loop e [] []
