...
type context = {
	...
	mutable custom_targets : (string * string option) list;
...
let add_custom_target com name path =
	let has_path = path <> None in
	List.iter (fun (n,p) ->
		if name = n then
			raise (Arg.Bad (Printf.sprintf "--custom-target of name \"%s\" added multiple times" name))
		else if has_path && (p <> None) then
			raise (Arg.Bad ("only one --custom-target may generate code; " ^
				Printf.sprintf "however, both \"%s\" and \"%s\" define an output path" name n))
	) com.custom_targets;
	com.custom_targets <- (name, path) :: com.custom_targets

let find_custom_target_generator com =
	let ct = List.find_opt (fun (n,p) -> p <> None) com.custom_targets in
	Option.map (fun (n,p) -> (n, Option.get p)) ct
...
let cache_directory ctx class_path dir f_dir =
	...
	and custom_exts = List.map (fun (n,p) -> "." ^ n) ctx.custom_targets
	...
	let prepare_file file_own_name =
		...
		let is_platform_specific,representation =
			...
			if is_loading_core_api then
				...
				(*
					The file contains double extension and the secondary one matches either:
					- the current platform (macro, js, ...)
					- a user-defined --custom-target
				*)
				if (platform_ext = second_ext) || (List.exists (fun cext -> second_ext = cext) custom_exts) then
