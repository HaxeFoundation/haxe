...
let make_module ctx mpath file loadp =
	let m = {
		...
		m_path = mpath;
		m_types = [];
		m_extra = module_extra (Path.get_real_path file) (Define.get_signature ctx.com.defines) (file_time file) (if ctx.in_macro then MMacro else MCode) (get_policy ctx mpath);
	} in
	m
