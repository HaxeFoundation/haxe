	...
	*)
	let create_module_types ctx m tdecls loadp =
		...
		let rec make_decl acc decl =
			...
			| EImport _ | EUsing _ ->
				if !has_declaration then str_typing_error "import and using may not appear after a declaration" p;
				acc
			| EStatic d ->
			...
			| EClass d ->
				...
				c.cl_build <- (fun() -> str_typing_error (s_type_path c.cl_path ^ " is not ready to be accessed, separate your type declarations in several files") p);
				c.cl_module <- m;
				c.cl_private <- priv;
				c.cl_doc <- d.d_doc;
				c.cl_meta <- d.d_meta;
				...
				if has_class_flag c CAbstract then begin
					if has_class_flag c CInterface then display_str_error ctx.com "An interface may not be abstract" c.cl_name_pos;
					if has_class_flag c CFinal then display_str_error ctx.com "An abstract class may not be final" c.cl_name_pos;
				end;
			...
			| EEnum d ->
				...
				if Meta.has (Meta.Custom ":fakeEnum") d.d_meta then str_typing_error "@:fakeEnum enums is no longer supported in Haxe 4, use extern enum abstract instead" p;
				let e = {
					e_path = path;
					e_module = m;
					e_pos = p;
		...
		List.fold_left (fun acc path ->
			...
			with Not_found ->
				if Sys.file_exists path then begin
					...
					in
					List.iter (fun (d,p) -> match d with EImport _ | EUsing _ -> () | _ -> str_typing_error "Only import and using is allowed in import.hx files" p) r;
		 ...
		 List.iter (fun d ->
			...
			| (TClassDecl c, (EClass d, p)) ->
				...
				if Meta.has Meta.GenericBuild c.cl_meta then begin
					if ctx.com.is_macro_context then str_typing_error "@:genericBuild cannot be used in macros" c.cl_pos;
...
module TypeLevel = struct
		...
		let rt = (match c.ec_type with
			...
			| Some (t,pt) ->
				...
				| TEnum (te,_) when te == e ->
					()
				| _ ->
					str_typing_error "Explicit enum type must be of the same enum type" pt);
				t
		) in
		let t = (match c.ec_args with
			...
			| l ->
				...
				TFun (List.map (fun (s,opt,(t,tp)) ->
					(match t with CTPath({tpackage=[];tname="Void"}) -> str_typing_error "Arguments of type Void are not allowed in enum constructors" tp | _ -> ());
					if PMap.mem s (!pnames) then str_typing_error ("Duplicate argument `" ^ s ^ "` in enum constructor " ^ fst c.ec_name) p;
		...
		let build() =
			...
			let rec build() =
				...
				with TypeloadCheck.Build_canceled state ->
					...
					| Built -> die "" __LOC__
					| Building cl ->
						if !build_count = !prev_build_count then str_typing_error ("Loop in class building prevent compiler termination (" ^ String.concat "," (List.map (fun c -> s_type_path c.cl_path) cl) ^ ")") c.cl_pos;
	...
	let init_enum ctx context_init e d p =
		...
		| None -> ()
		| Some (h,hcl) ->
			Hashtbl.iter (fun _ _ -> str_typing_error "Field type patch not supported for enums" e.e_pos) h;
		...
		TypeloadFields.build_module_def ctx (TEnumDecl e) e.e_meta get_constructs context_init (fun (e,p) ->
			...
			| EVars [{ ev_type = Some (CTAnonymous fields,p); ev_expr = None }] ->
				constructs := List.map (fun f ->
					...
					| FFun { f_params = pl; f_type = t; f_expr = (None|Some (EBlock [],_)); f_args = al } ->
						let al = List.map (fun ((n,_),o,_,t,_) -> match t with None -> str_typing_error "Missing function parameter type" f.cff_pos | Some t -> n,o,t) al in
						al, pl, t
					| _ ->
						str_typing_error "Invalid enum constructor in @:build result" p
					) in
					{
						ec_name = f.cff_name;
						ec_doc = f.cff_doc;
			...
			| _ -> str_typing_error "Enum build macro must return a single variable with anonymous object fields" p
		);
		...
		List.iter (fun c ->
			if PMap.mem (fst c.ec_name) e.e_constrs then str_typing_error ("Duplicate constructor " ^ fst c.ec_name) (pos c.ec_name);
	...
	let init_typedef ctx context_init t d p =
		...
		| CTPath { tpackage = ["haxe";"macro"]; tname = "MacroType" } ->
			...
			if t.t_type == follow tt then str_typing_error "Recursive typedef is not allowed" p;
			tt
		| _ ->
			if (Meta.has Meta.Eager d.d_meta) then
				...
				let rec check_rec tt =
					if tt == t.t_type then str_typing_error "Recursive typedef is not allowed" p;
					match tt with
					| TMono r ->
					...
					| TType (td,tl) ->
						if td == t then str_typing_error "Recursive typedef is not allowed" p;
	...
	let init_abstract ctx context_init a d p =
			...
			let t = if not (Meta.has Meta.CoreType a.a_meta) then begin
				if !is_type then begin
					let r = exc_protect ctx (fun r ->
						...
						(try (if from then Type.unify t a.a_this else Type.unify a.a_this t) with Unify_error _ -> str_typing_error "You can only declare from/to with compatible types" pos);
						t
					) "constraint" in
					TLazy r
				end else
					str_typing_error "Missing underlying type declaration or @:coreType declaration" p;
			end else begin
				if Meta.has Meta.Callable a.a_meta then
					str_typing_error "@:coreType abstracts cannot be @:callable" p;
				t
			end in
			t
		in
		List.iter (function
			...
			| AbOver t ->
				if a.a_impl = None then str_typing_error "Abstracts with underlying type must have an implementation" a.a_pos;
				if Meta.has Meta.CoreType a.a_meta then str_typing_error "@:coreType abstracts cannot have an underlying type" p;
					...
					let rec loop stack t =
						...
						| TAbstract(a,_) when not (Meta.has Meta.CoreType a.a_meta) ->
							if List.memq a stack then
								str_typing_error "Abstract underlying type cannot be recursive" a.a_pos
							else
		...
		if not !is_type then begin
			if Meta.has Meta.CoreType a.a_meta then
				...
				str_typing_error "Abstract is missing underlying type declaration" a.a_pos
		end;
