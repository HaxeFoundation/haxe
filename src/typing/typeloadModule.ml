...
let init_module_type ctx context_init do_init (decl,p) =
	...
	| EImport (path,mode) ->
		...
		| [] ->
			...
			| _ ->
				...
				| [] -> DisplayException.raise_fields (DisplayToplevel.collect ctx None NoValue) CRImport None false;
	...
	| EUsing path ->
		...
		let t = match List.rev path with
			...
			| [] ->
				DisplayException.raise_fields (DisplayToplevel.collect ctx None NoValue) CRUsing None false;
		in
		(* do the import first *)
