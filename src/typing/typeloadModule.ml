...
module ModuleLevel = struct
	let make_module com g mpath file loadp =
		let m = {
			...
			m_types = [];
			m_statics = None;
			m_extra = module_extra (Path.get_full_path file) (Define.get_signature com.defines) (file_time file) (if com.is_macro_context then MMacro else MCode) (get_policy g mpath);
		} in
		m
...
let create_typer_context_for_module com g m = {
		...
		m = {
			curmod = m;
			module_imports = List.map (fun t -> t,null_pos) g.std.m_types;
			module_using = [];
