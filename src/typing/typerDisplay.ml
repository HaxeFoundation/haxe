...
let completion_item_of_expr ctx e =
	...
	let rec loop e = match e.eexpr with
		...
		| TTypeExpr (TClassDecl {cl_kind = KAbstractImpl a}) ->
			...
			let t = TType(abstract_module_type a (List.map hack_tp a.a_params),[]) in
			let t = tpair t in
			...
			| ITType({kind = (Class | Abstract | TypeAlias)} as mt,_) when not mt.is_private || is_private_to_current_module mt ->
				...
				| Maybe ->
					begin try
						...
						| TAbstractDecl a -> (match Abstract.follow_with_forward_ctor ~build:true (TAbstract(a,List.map hack_tp a.a_params)) with
