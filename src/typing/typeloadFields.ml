...
let create_typer_context_for_class ctx cctx p =
	...
	if cctx.is_lib && not (has_class_flag c CExtern) then ctx.com.error (located "@:libType can only be used in extern classes" c.cl_pos);
		...
		try
			...
			| FKConstructor ->
				...
				| Some ({ cl_constructor = Some ctor_sup } as c, _) when not (has_class_flag c CExtern) && has_class_field_flag ctor_sup CfFinal ->
					ctx.com.error (located "Cannot override final constructor" cf.cf_pos)
				| _ -> ()
				end;
