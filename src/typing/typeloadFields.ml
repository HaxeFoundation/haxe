		...
		| FVar(ct,eo) when not (List.mem_assoc AStatic field.cff_access) && not (field_is_set field) ->
			let check_visibility_conflict visibility p1 =
				...
				| VPublic(access,p2) | VPrivate(access,p2) ->
					...
					display_str_error ~depth:1 ctx.com "Conflicts with this" p2;
			in
...
let build_module_def ctx mt meta fvars context_init fbuild =
	...
	let loop f_build = function
		| Meta.Build,args,p when not is_typedef -> (fun () ->
				...
				let s = try String.concat "." (List.rev (string_list_of_expr_path epath)) with Error (_,p,depth) -> str_typing_error ~depth "Build call parameter must be a class path" p in
		...
		| Some a ->
			...
			let rec loop ml =
				...
				| (Meta.To,_,_) :: _ ->
					...
					let resolve_m args =
						(try unify_raise t (tfun (tthis :: args) m) cf.cf_pos with Error (Unify l,p,depth) -> typing_error ~depth (error_msg p (Unify l)));
						match follow m with
				...
				try
					(match f2.cf_kind with
						| Method MethMacro ->
							...
							display_str_error ~depth:1 ctx.com (compl_msg (f2.cf_name ^ ": Accessor method is here")) f2.cf_pos;
						| _ -> ());
...
let check_overload ctx f fs =
	try
		...
		display_str_error ~depth:1 ctx.com (compl_msg "The second field is declared here") f2.cf_pos;
		false
	with Not_found -> try
...
let init_class ctx c p context_init herits fields =
	...
	List.iter (fun f ->
		...
		with Error (Custom str,p2,depth) when p = p2 ->
			display_str_error ~depth ctx.com str p
	) fields;
	(match cctx.abstract with
	| Some a ->
