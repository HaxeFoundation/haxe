...
let check_constructor_access ctx c f p =
	if (Meta.has Meta.CompilerGenerated f.cf_meta) then display_error_msg ctx.com (error_msg (No_constructor (TClassDecl c))) p;
	if not (can_access ctx c f true || extends ctx.curclass c) && not ctx.untyped then display_error_msg ctx.com (Printf.sprintf "Cannot access private constructor of %s" (s_class_path c)) p
...
let field_access ctx mode f fh e pfield =
	...
	| Method m ->
		...
		let default () =
			...
			| MethMacro, MGet ->
				display_error_msg ctx.com "Macro functions must be called immediately" pfield; normal()
			| _ , MGet ->
				if has_class_field_flag f CfGeneric then display_error_msg ctx.com "Cannot create closure on generic function" pfield;
				normal()
			| _ ->
				normal()
		in
		...
		| FHInstance(c,tl) ->
			if e.eexpr = TConst TSuper then begin match mode with
				| MSet _ | MGet ->
					display_error_msg ctx.com "Cannot create closure on super method" pfield
				| MCall _ ->
					()
			end;
		...
		| FHInstance _ ->
			...
			if e.eexpr = TConst TSuper then begin match mode with
				...
				| MSet _ when v.v_write = AccCall ->
					()
				| _ ->
					display_error_msg ctx.com "Normal variables cannot be accessed with 'super', use 'this' instead" pfield;
			end;
		| FHAnon ->
			()
		end;
		...
		| AccCall ->
			...
			if bypass_accessor then (
				...
				if not (is_physical_field f) then begin
					display_error_msg ctx.com "This field cannot be accessed because it is not a real variable" pfield;
					display_error_msg ctx.com "Add @:isVar here to enable it" f.cf_pos;
				end;
				normal false
			)
	...
	let rec type_field_by_type e t =
		...
		| TAnon a ->
			(try
				...
				if has_class_field_flag f CfImpl && not (has_class_field_flag f CfEnum) then display_error_msg ctx.com "Cannot access non-static abstract field statically" pfield;
				match !(a.a_status) with
				| EnumStatics en ->
	...
	with Not_found when not (TypeFieldConfig.do_resume cfg) ->
		if not ctx.untyped then begin
			...
			| TAbstract (a,_) when has_special_field a ->
				...
				display_error_msg ctx.com ("Field " ^ i ^ " cannot be called directly because it has no expression") pfield;
			| TAnon { a_status = { contents = Statics c } } when PMap.mem i c.cl_fields ->
				display_error_msg ctx.com ("Static access to instance field " ^ i ^ " is not allowed") pfield;
			| _ ->
				...
				with Exit ->
					display_error_msg ctx.com (StringError.string_error i (string_source tthis) (s_type (print_context()) tthis ^ " has no field " ^ i)) pfield
		end;
