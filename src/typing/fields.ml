...
let field_access ctx mode f fh e pfield =
	...
	let make_access inline = FieldAccess.create e f fh (inline && ctx.g.allow_inline) pfull in
	match f.cf_kind with
	| Method m ->
		...
		let default () =
			match m, mode with
			| MethInline, _ when ctx.g.doinline && ctx.g.allow_inline ->
	...
	| Var v ->
		...
		| AccNormal | AccNo ->
			normal false
		| AccCall when (not ctx.g.allow_transform) || (ctx.f.in_display && DisplayPosition.display_position#enclosed_in pfull) ->
			normal false
		| AccCall ->
