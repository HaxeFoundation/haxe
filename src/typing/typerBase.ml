...
let is_lower_ident s p =
	...
	with Invalid_argument msg -> str_typing_error msg p

let get_this ctx p =
	match ctx.curfun with
	| FunStatic ->
		str_typing_error "Cannot access this from a static function" p
	...
	| FunMemberAbstract ->
		let v = (try PMap.find "this" ctx.locals with Not_found -> str_typing_error "Cannot reference this abstract here" p) in
...
let rec type_module_type ctx t tparams p =
	...
	| TClassDecl {cl_kind = KGenericBuild _} ->
		...
		with Exit ->
			...
			else str_typing_error "Invalid module type" p
		in
	...
	| TTypeDecl s ->
		...
		| _ ->
			str_typing_error (s_type_path s.t_path ^ " is not a value") p)
	| TAbstractDecl { a_impl = Some c } ->
		type_module_type ctx (TClassDecl c) tparams p
	| TAbstractDecl a ->
		if not (Meta.has Meta.RuntimeValue a.a_meta) then str_typing_error (s_type_path a.a_path ^ " is not a value") p;
...
let get_constructible_constraint ctx tl p =
	let extract_function t = match follow t with
		| TFun(tl,tr) -> tl,tr
		| _ -> str_typing_error "Constructible type parameter should be function" p
	in
