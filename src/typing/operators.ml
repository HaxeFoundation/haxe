...
let compare_functions_by_reflect ctx op e1 e2 p =
	let reflect_cls =
		...
		| TClassDecl cls -> cls
		| _ -> typing_error "Reflect is expected to be a class" p
	in
	let compareMethods_field =
		try PMap.find "compareMethods" reflect_cls.cl_statics
		with Not_found -> typing_error ("Reflect has no field compareMethods") p
	in
	...
	let declarations,comparison =
		...
		| _ ->
			...
			and isFunction_field =
				...
				with Not_found -> typing_error ("Reflect has no field isFunction") p
			in
