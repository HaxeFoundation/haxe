...
let check_native_name_override ctx child base =
	let error base_pos child_pos =
		...
		display_error_msg ctx.com ("Field " ^ child.cf_name ^ " has different @:native value than in superclass") child_pos;
		display_error_msg ~depth:1 ctx.com (compl_msg "Base field is defined here") base_pos
	in
	try
...
let check_overriding ctx c f =
	...
	| None ->
		if has_class_field_flag f CfOverride then display_error_msg ctx.com ("Field " ^ f.cf_name ^ " is declared 'override' but doesn't override any field") f.cf_pos
	...
	| Some (csup,params) ->
		...
		let check_field f get_super_field is_overload = try
			(if is_overload && not (has_class_field_flag f CfOverload) then
				display_error_msg ctx.com ("Missing overload declaration for field " ^ i) p);
			...
			if has_class_field_flag f2 CfAbstract then begin
				if f_has_override then
					display_error_msg ctx.com ("Field " ^ i ^ " is declared 'override' but parent field " ^ i ^ " is 'abstract' and does not provide any implementation to override") p
				else
			...
			if (has_class_field_flag f2 CfOverload && not (has_class_field_flag f CfOverload)) then
				display_error_msg ctx.com ("Field " ^ i ^ " should be declared with overload since it was already declared as overload in superclass") p
				...
				else display_error_msg ctx.com ("Field " ^ i ^ " should be declared with 'override' since it is inherited from superclass " ^ s_type_path csup.cl_path) p
			end else if not (has_class_field_flag f CfPublic) && (has_class_field_flag f2 CfPublic) then
				display_error_msg ctx.com ("Field " ^ i ^ " has less visibility (public/private) than superclass one") p
			else (match f.cf_kind, f2.cf_kind with
			| _, Method MethInline ->
				display_error_msg ctx.com ("Field " ^ i ^ " is inlined and cannot be overridden") p
			| a, b when a = b -> ()
			| Method MethInline, Method MethNormal ->
				() (* allow to redefine a method as inlined *)
			| _ ->
				display_error_msg ctx.com ("Field " ^ i ^ " has different property access than in superclass") p);
			if (has_class_field_flag f2 CfFinal) then display_error_msg ctx.com ("Cannot override final method " ^ i) p;
			try
			...
			with
				Unify_error l ->
					...
					display_error_msg ctx.com ("Field " ^ i ^ " overrides parent class with different or incomplete type") p;
					display_error_msg ~depth:1 ctx.com (compl_msg "Base field is defined here") f2.cf_name_pos;
					display_error_msg ~depth:1 ctx.com (compl_msg (error_msg (Unify l))) p;
		with
			Not_found ->
		...
		if has_class_field_flag f CfOverload then begin
			...
			List.iter (fun (t,f2) ->
				...
				match f2.cf_kind with
				| Var _ ->
					display_error_msg ctx.com ("A variable named '" ^ f2.cf_name ^ "' was already declared in a superclass") f.cf_pos
				| _ -> ()
			) overloads;
			List.iter (fun f ->
...
let rec return_flow ctx e =
	let error() =
		display_error_msg ctx.com (Printf.sprintf "Missing return: %s" (s_type (print_context()) ctx.ret)) e.epos; raise Exit
	in
...
module Inheritance = struct
			...
			try
				...
				delay ctx PForce (fun () ->
					...
					if (has_class_field_flag f CfPublic) && not (has_class_field_flag f2 CfPublic) && not (Meta.has Meta.CompilerGenerated f.cf_meta) then
						display_error_msg ctx.com ("Field " ^ f.cf_name ^ " should be public as requested by " ^ s_type_path intf.cl_path) p
						...
						display_error_msg ctx.com ("Field " ^ f.cf_name ^ " has different property access than in " ^ s_type_path intf.cl_path ^ " (" ^ s_kind f2.cf_kind ^ " should be " ^ s_kind f.cf_kind ^ ")") p
					else try
					...
					with
						Unify_error l ->
							if not (Meta.has Meta.CsNative c.cl_meta && (has_class_flag c CExtern)) then begin
								(* TODO construct error with sub *)
								display_error_msg ctx.com ("Field " ^ f.cf_name ^ " has different type than in " ^ s_type_path intf.cl_path) p;
								display_error_msg ~depth:1 ctx.com (compl_msg "Interface field is defined here") f.cf_pos;
								display_error_msg ~depth:1 ctx.com (compl_msg (error_msg (Unify l))) p;
							end
				)
			with Not_found ->
	...
	let check_abstract_class ctx c csup params =
		...
		| l ->
			...
			display_error_msg ctx.com (Printf.sprintf "This class extends abstract class %s but doesn't implement the following method%s" (s_type_path csup.cl_path) (if singular then "" else "s")) c.cl_name_pos;
			(* TODO sub error ? *)
			display_error_msg ctx.com (Printf.sprintf "Implement %s or make %s abstract as well" (if singular then "it" else "them") (s_type_path c.cl_path)) c.cl_name_pos;
			...
			List.iter (fun (cf,_) ->
				let s = match follow cf.cf_type with
					...
					| t ->
						s_type pctx t
				in
				display_error_msg ~depth:1 ctx.com (compl_msg (Printf.sprintf "%s(%s)" cf.cf_name s)) cf.cf_name_pos
			) (List.rev !missing)
		...
		let check_herit t is_extends p =
			...
			if is_extends then begin
				...
				| TDynamic t ->
					...
					if not (has_class_flag c CExtern) then display_error_msg ctx.com "In haxe 4, implements Dynamic is only supported on externs" p;
...
let check_final_vars ctx e =
	...
	if Hashtbl.length final_vars > 0 then begin
		...
		if Hashtbl.length final_vars > 0 then
			display_error_msg ctx.com "Some final fields are uninitialized in this class" ctx.curclass.cl_name_pos;
		DynArray.iter (fun (c,cf) ->
			if Hashtbl.mem final_vars cf.cf_name then
				display_error_msg ~depth:1 ctx.com "Uninitialized field" cf.cf_name_pos
		) ordered_fields
	end
