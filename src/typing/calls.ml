...
let type_bind ctx (e : texpr) (args,ret) params p =
	...
	let e,var_decls =
		...
		| TField(eobj,FClosure(Some (cl,tp), cf)) when is_immutable_method cf ->
			...
			*)
			let v_this = alloc_var VGenerated "`" eobj.etype eobj.epos in
			let e_var_decl = mk (TVar(v_this,Some eobj)) ctx.t.tvoid eobj.epos in (* TODO: can we be smarter and only tempvar object when necessary? *)
			let e_local = mk (TLocal v_this) eobj.etype eobj.epos in
			let e = { e with eexpr = TField(e_local, FInstance (cl, tp, cf)) } in
			e, e_var_decl :: var_decls
		| _ ->
