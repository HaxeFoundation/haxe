...
let resolve_module_file com m remap p =
	...
	(match fst m with
	| "std" :: _ ->
		let file = Path.get_real_path file in
		if List.exists (fun path -> ExtString.String.starts_with file (try Path.get_real_path path with _ -> path)) com.std_path then raise Not_found;
	| _ -> ());
