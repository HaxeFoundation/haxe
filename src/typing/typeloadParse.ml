...
let parse_module ctx m p =
	...
	if pack <> !remap then begin
		...
		if p == null_pos then
			display_error_msg ctx.com ("Invalid commandline class : " ^ s_type_path m ^ " should be " ^ s_type_path (pack,snd m)) p
		else
			display_error_msg ctx.com (spack pack ^ " in " ^ file ^ " should be " ^ spack (fst m)) {p with pmax = p.pmin}
	end;
