	...
	object (self)
		...
		*)
		...
		method private check_fields_initialization fields_to_initialize tf_expr is_static =
			(* Check if expression ends with throw (doesn't return an object, no initialization needed) *)
			let rec ends_with_throw e =
				match e.eexpr with
					| TThrow _ -> true
					| TBlock exprs -> (match List.rev exprs with [] -> false | last :: _ -> ends_with_throw last)
					| TIf (_, if_body, Some else_body) -> ends_with_throw if_body && ends_with_throw else_body
					| TParenthesis e | TMeta (_, e) -> ends_with_throw e
					| _ -> false
			in
			(* Check if expression ends with return (returns an object, all fields must be initialized) *)
			let rec ends_with_return e =
				match e.eexpr with
					| TReturn _ -> true
					| TBlock exprs -> (match List.rev exprs with [] -> false | last :: _ -> ends_with_return last)
					| TIf (_, if_body, Some else_body) -> ends_with_return if_body && ends_with_return else_body
					| TParenthesis e | TMeta (_, e) -> ends_with_return e
					| _ -> false
			in
			...
			let rec traverse init_list e =
				(match e.eexpr with
					...
					| TIf (_, if_block, Some else_block) ->
						...
						let if_throws = ends_with_throw if_block in
						let else_throws = ends_with_throw else_block in
						let if_returns = ends_with_return if_block in
						let else_returns = ends_with_return else_block in
						Hashtbl.clear init_list;
						(* Branch ending with throw: doesn't return an object, so field initialization doesn't matter
						   Branch ending with return: returns an object, so all fields must be initialized
						   Branch with neither: continues normally, so uninitialized fields propagate *)
						if if_throws && else_throws then
							(* Both branches throw, constructor never returns, no fields need initialization *)
							()
						else if if_throws then
							(* If branch throws (ignored), only consider else branch *)
							if else_returns then
								(* Else returns: must have all fields initialized, report any that aren't *)
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
							else
								(* Else continues normally: propagate its uninitialized fields *)
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
						else if else_throws then
							(* Else branch throws (ignored), only consider if branch *)
							if if_returns then
								(* If returns: must have all fields initialized, report any that aren't *)
								Hashtbl.iter (Hashtbl.replace init_list) if_init_list
							else
								(* If continues normally: propagate its uninitialized fields *)
								Hashtbl.iter (Hashtbl.replace init_list) if_init_list
						else
							(* Neither branch throws *)
							if if_returns && else_returns then
								(* Both return: all fields must be initialized in both branches *)
								Hashtbl.iter (Hashtbl.replace init_list) if_init_list;
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
							else if if_returns then
								(* If returns, else continues: if must have all fields initialized, else propagates *)
								Hashtbl.iter (Hashtbl.replace init_list) if_init_list;
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
							else if else_returns then
								(* Else returns, if continues: else must have all fields initialized, if propagates *)
								Hashtbl.iter (Hashtbl.replace init_list) if_init_list;
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
							else
								...
								Hashtbl.iter (Hashtbl.replace init_list) else_init_list
					| TIf (_, if_block, None) ->
						(* If without else - need to check if it returns/throws *)
						let if_throws = ends_with_throw if_block in
						let if_returns = ends_with_return if_block in
						if if_returns then
							(* If returns: all fields must be initialized before return *)
							let if_init_list = traverse (Hashtbl.copy init_list) if_block in
							(* Check if any fields are still uninitialized in the if branch *)
							Hashtbl.iter (fun name field ->
								if Hashtbl.mem if_init_list name then
									checker#error
										("Field \"" ^ name ^ "\" is not nullable thus should have an initial value or should be initialized before return.")
										[field.cf_pos]
							) init_list
						else if not if_throws then
							(* If continues normally or throws: just traverse it normally *)
							ignore (traverse init_list if_block)
					(* var _gthis = this *)
