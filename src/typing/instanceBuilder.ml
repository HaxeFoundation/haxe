...
let build_macro_type ctx pl p =
	let path, field, args = (match pl with
		...
		| _ ->
			str_typing_error "MacroType requires a single expression call parameter" p
	) in
	let old = ctx.ret in
...
let build_macro_build ctx c pl cfl p =
	let path, field, args =
		let build_expr =
			...
			with Not_found -> str_typing_error ((s_type_path c.cl_path) ^ " is missing @:genericBuild meta. Was it removed by a macro?") p
		in
		match build_expr with
		| _,[ECall(e,args),_],_ -> get_macro_path ctx e args p
		| _ -> str_typing_error "genericBuild requires a single expression call parameter" p
	in
