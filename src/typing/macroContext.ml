	...
	in
	{
		...
		MacroApi.define_type = (fun v mdep ->
			...
			let add is_macro ctx =
				...
				let mnew = TypeloadModule.type_module ctx ctx.g ~dont_check_path:(has_native_meta) m (Path.UniqueKey.lazy_path mdep.m_extra.m_file) [tdef,pos] pos in
		...
		MacroApi.define_module = (fun m types imports usings ->
			...
			with Not_found ->
				let mnew = TypeloadModule.type_module ctx ctx.g mpath (Path.UniqueKey.lazy_path ctx.m.curmod.m_extra.m_file) types pos in
		...
		MacroApi.display_error = Typecore.display_error ctx.com;
