	...
	| _ ->
		...
		with Exit ->
			...
			| UnifyMinOk t ->
				t
			| UnifyMinError(l,index) ->
				raise_typing_error (make_error (Unify l) (List.nth el index).epos)
			end

let unify_min ctx el =
	try unify_min_raise ctx el
	with Error ({ err_message = Unify l } as err) ->
		if not ctx.untyped then display_error ctx.com err;
		(List.hd el).etype
		...
		with Not_found ->
			if ctx.untyped then begin
				...
				if !resolved_to_type_parameter then begin
					display_error_msg ctx.com ("Only @:const type parameters on @:generic classes can be used as value") p;
					...
					if Diagnostics.error_in_diagnostics_run ctx.com p then begin
						...
						| DMNone ->
							raise_error_msg err p
						| _ ->
							display_error_msg ctx.com (error_msg err) p;
							let t = mk_mono() in
...
and type_new ctx path el with_type force_inline p =
	...
	| TInst ({cl_kind = KTypeParameter tl} as c,params) ->
		...
		| None ->
			raise_typing_error (make_error (No_constructor (TClassDecl c)) p)
		| Some(tl,tr) ->
...
and type_try ctx e1 catches with_type p =
	...
	let rec check_unreachable cases t p = match cases with
		| (v,e) :: cases ->
			let unreachable () =
				display_error_msg ctx.com "This block is unreachable" p;
				let st = s_type (print_context()) in
				display_error_msg ctx.com (Printf.sprintf "%s can be caught to %s, which is handled here" (st t) (st v.v_type)) e.epos
			in
			begin try
...
and type_map_declaration ctx e1 el with_type p =
	...
	let check_key e_key =
		try
			...
			display_error_msg ctx.com "Duplicate key" e_key.epos;
...
and type_local_function ctx kind f with_type p =
	...
	if params <> [] then begin
		if name = None then display_error_msg ctx.com "Type parameters not supported in unnamed local functions" p;
	...
	*)
	...
	| WithType.WithType(t,_) ->
		...
		loop [] t
	| WithType.NoValue ->
		if name = None then display_error_msg ctx.com "Unnamed lvalue functions are not supported" p
	| _ ->
		());
	...
	| Some v ->
		...
		let exprs =
			if is_rec then begin
				if inline then display_error_msg ctx.com "Inline function cannot be recursive" e.epos;
	...
	| None ->
		...
		with Error ({ err_message = Unify _ } as err) ->
			if !allow_array_dynamic || ctx.untyped || ignore_error ctx.com then
				...
				display_error_msg ctx.com "Arrays of mixed types are only allowed if the type is forced to Array<Dynamic>" err.err_pos;
				raise_error err
			end
		in
...
and type_return ?(implicit=false) ctx e with_type p =
	...
	| Some e ->
		if is_abstract_ctor then begin
			...
			| _ -> display_error_msg ctx.com "Cannot return a value from constructor" p
		end;
		try
...
and type_meta ?(mode=MGet) ctx m e1 with_type p =
	...
	let e = match m with
		...
		| (Meta.BypassAccessor,_,p) ->
			...
			let e = e () in
			(if ctx.bypass_accessor > old_counter then display_error_msg ctx.com "Field access expression expected after @:bypassAccessor metadata" p);
			e
		| (Meta.Inline,_,pinline) ->
			...
			| _ ->
				display_error_msg ctx.com "Call or function expected after inline keyword" p;
				e();
			end
		...
		| (Meta.Dollar s,_,p) ->
			display_error_msg ctx.com (Printf.sprintf "Reification $%s is not allowed outside of `macro` expression" s) p;
			e()
		| _ ->
...
and type_call_target ctx e el with_type p_inline =
	...
	let check_inline cf p =
		if (has_class_field_flag cf CfAbstract) then display_error_msg ctx.com "Cannot force inline on abstract method" p
	in
	match p_inline with
	| None ->
		e
	| Some pinline ->
		let rec loop e =
			...
			| AKExpr {eexpr = TLocal _} ->
				display_error_msg ctx.com "Cannot force inline on local functions" pinline;
				e
			| _ ->
				e
		in
...
and type_call_builtin ctx e el mode with_type p =
	...
	| (EConst (Ident "super"),sp) , el ->
		...
		| Some (c,params) ->
			...
			if (Meta.has Meta.CompilerGenerated cf.cf_meta) then display_error_msg ctx.com (error_msg (No_constructor (TClassDecl c))) p;
...
and type_expr ?(mode=MGet) ctx (e,p) (with_type:WithType.t) =
	...
	| EReturn e ->
		if not ctx.in_function then begin
			display_error_msg ctx.com "Return outside function" p;
			match e with
			| None ->
	...
	| EBreak ->
		if not ctx.in_loop then display_error_msg ctx.com "Break outside loop" p;
		mk TBreak (mono_or_dynamic ctx with_type p) p
	| EContinue ->
		if not ctx.in_loop then display_error_msg ctx.com "Continue outside loop" p;
	...
	| EIs (e,(t,p_t)) ->
		match t with
		| CTPath tp ->
			if tp.tparams <> [] then display_error_msg ctx.com "Type parameters are not supported for the `is` operator" p_t;
			let e = type_expr ctx e WithType.value in
		...
		| _ ->
			display_error_msg ctx.com "Unsupported type for `is` operator" p_t;
