...
let rec process_meta_argument ?(toplevel=true) ctx expr = match expr.eexpr with
	...
	| _ ->
		display_error_msg ctx.com "This expression is too complex to be a strict metadata argument" expr.epos;
		(EConst(Ident "null"), expr.epos)
...
let make_meta ctx texpr extra =
	match texpr.eexpr with
		...
		| _ ->
			display_error_msg ctx.com "Unexpected expression" texpr.epos; die "" __LOC__
	...
	let changed_expr, fields_to_check, ctype = match params with
		| [ECall(ef, el),p] ->
			...
			| Java ->
				...
				| [] ->
					[]
				| (_,p) :: _ ->
					display_error_msg ctx.com "Object declaration expected" p;
					[]
				in
		...
		| [ (EField(_),p as field) ] ->
			...
			if pf = Cs then
				...
				field, [], CTPath tpath
		| _ ->
			display_error_msg ctx.com "A @:strict metadata must contain exactly one parameter. Please check the documentation for more information" pos;
			raise Exit
	in
