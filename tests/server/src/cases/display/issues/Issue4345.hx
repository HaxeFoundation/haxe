	...
	**/
	function test(_) {
		...
		assertHasCompletion(result, item -> switch item.kind {
			case ClassField: item.args.field.name == "name" && item.args.field.type.args.path.typeName == "String";
			case _: false;
		});
		assertHasCompletion(result, item -> switch item.kind {
			case ClassField: item.args.field.name == "count" && item.args.field.type.args.path.typeName == "Int";
			case _: false;
		});
	}
}
