...
class Issue8805 extends DisplayTestCase {
	function testIssue8805_gotoAbstractPropertyWithInlineGetter(_) {
		...
		var args = ["-main", "Main"];
		var result = runHaxeJson(args, DisplayMethods.GotoDefinition, {file: file, offset: 56});
		if (result.length == 0) {
			Assert.fail('display/definition failed');
		} else {
			Assert.same({"start": {"line": 7, "character": 12}, "end": {"line": 7, "character": 15}}, result[0].range);
		}
	}
}