...
class TestUInt32 extends unit.Test {
	...
	function testToString() {
		eq(Std.string(UInt32.ofInt(0)), "0");
		eq(Std.string(UInt32.ofInt(1)), "1");
		eq(Std.string(UInt32.ofInt(100)), "100");
		eq(Std.string(UInt32.ofInt(2147483647)), "2147483647"); // Int.MAX
	...
	function testDiv() {
		...
		// divMod
		var result = UInt32.divMod(cast(47, UInt32), cast(5, UInt32));
		eq(Std.string(result.quotient), "9");
		eq(Std.string(result.modulus), "2");
	}

	function testDivByZero() {
		var threw = false;
		try {
			UInt32.divMod(ONE, ZERO);
		} catch (e:Dynamic) {
			threw = true;
		}
		t(threw);
	...
	function testShl() {
		...
		var r = ONE << 31;
		eq(Std.string(r), "2147483648"); // 0x80000000 = HIGH
		t(r == HIGH);
	}
	...
	// --- fromInt32 / toInt32 round-trip ---
	function testInt32Conversion() {
		// Positive values: round-trip
		var u:UInt32 = cast 42;
		var i:haxe.Int32 = u.toInt32();
		eq((i : Int), 42);
		t(UInt32.fromInt32(i) == u);

		// Bit-identical: 0xFFFFFFFF as UInt32 == -1 as Int32
		var u2:UInt32 = MAX;
		var i2:haxe.Int32 = u2.toInt32();
		eq((i2 : Int), -1);
		t(UInt32.fromInt32(i2) == u2);
	}
