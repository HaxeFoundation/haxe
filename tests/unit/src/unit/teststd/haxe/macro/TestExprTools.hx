...
class TestExprTools extends unit.Test {
	public function test() {
		...
		eq(map(econst, wrap).expr, econst.expr);
		eq(map(econtinue, wrap).expr, econtinue.expr);
		eq(map(ebreak, wrap).expr, ebreak.expr);
		check(efield);
		check(eparenthesis);
		check(euntyped);
		check(ethrow);
		...
		eq(1, getValue(macro 1));
		eq("1", getValue(macro "1"));
		feq(12.2, getValue(macro 12.2));
		t(getValue(macro true));
		f(getValue(macro false));
		eq(null, getValue(macro null));
		eq(null, getValue(macro (null)));
		eq(null, getValue(macro untyped null));
		eq(null, getValue(macro @:meta null));
		...
		eq(-1, getValue(macro -1));
		f(getValue(macro !true));
		t(getValue(macro !false));

		eq(5, getValue(macro 2 + 3));
		eq(-1, getValue(macro 2 - 3));
		eq(6, getValue(macro 2 * 3));
		eq(2, getValue(macro 4 / 2));
		eq(1, getValue(macro 3 % 2));
		t(getValue(macro 1 == 1));
		f(getValue(macro 1 != 1));
		t(getValue(macro 1 > 0));
		f(getValue(macro 0 > 1));
		t(getValue(macro 0 < 1));
		f(getValue(macro 1 < 0));
		t(getValue(macro 1 >= 0));
		t(getValue(macro 1 >= 1));
		f(getValue(macro 0 >= 1));
		t(getValue(macro 0 <= 1));
		t(getValue(macro 1 <= 1));
		f(getValue(macro 1 <= 0));
		eq(7, getValue(macro 5 | 2));
		eq(1, getValue(macro 5 & 3));
		eq(6, getValue(macro 5 ^ 3));
		t(getValue(macro true && true));
		f(getValue(macro true && false));
		t(getValue(macro true || false));
		f(getValue(macro false || false));
		eq(8, getValue(macro 4 << 1));
		eq(4, getValue(macro 8 >> 1));
		eq(4, getValue(macro 8 >>> 1));
	}
}
