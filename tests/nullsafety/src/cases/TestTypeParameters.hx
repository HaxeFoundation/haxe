...
// ---- Nullable values passed directly to Dynamic parameters ----

@:nullSafety
class TestNullableToDynamic {
	// Nullable argument to a Dynamic parameter should pass (issue #9588)
	static function passNullableToDynamicParam(?n:String) {
		acceptDynamic(n);
	}

	static function acceptDynamic(v:Dynamic):Void {}

	// Assigning nullable to a Dynamic variable should pass
	static function assignNullableToDynamic(?n:String) {
		var d:Dynamic = n;
	}

	// Returning a nullable value from a Dynamic-typed function should pass
	static function returnNullableAsDynamic(?n:String):Dynamic {
		return n;
	}

	// Std.string takes Dynamic, so nullable argument should pass
	static function passNullableToStdString(?n:String) {
		Std.string(n);
	}
}
...
// ---- Function types with Dynamic parameters ----

@:nullSafety
class TestFunctionTypeDynamic {
	// Calling a function typed as Dynamic->Void with a nullable should pass
	static function callDynamicFunctionWithNullable(?n:String) {
		var fn:(v:Dynamic) -> Void = v -> trace(v);
		fn(n);
	}

	// Higher-order: passing a Null<X>->Void where Dynamic->Void is expected
	static function nullableCallbackWhereDynamicExpected(?n:String) {
		var handler:((v:Dynamic) -> Void) -> Void = cb -> cb(n);
		handler(v -> trace(v));
	}
}
