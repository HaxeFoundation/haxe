...
static value *pcre2_exc_Error         = NULL;  /* Exception [Error] */
static value *pcre2_exc_Backtrack     = NULL;  /* Exception [Backtrack] */
...
static inline void copy_ovector(
  long subj_start, const size_t* ovec_src, ovec_dst_ptr ovec_dst, uint32_t subgroups2)
{
  if (subj_start == 0)
    while (subgroups2--) {
...
static void pcre2_dealloc_tables(value v_table)
{ pcre2_maketables_free(NULL, (void *) Field(v_table, 1)); }

/* Finalizing deallocation function for compiled regular expressions */
static void pcre2_dealloc_regexp(value v_rex)
{
  pcre2_code_free((pcre2_code *) Field(v_rex, 1));
  pcre2_match_context_free((pcre2_match_context *) Field(v_rex, 2));
}
...
CAMLprim value pcre2_version_stub(value __unused v_unit)
{
  ...
  pcre2_config(PCRE2_CONFIG_VERSION, String_val(v_version));
...
static inline void raise_pcre2_error(value v_arg) Noreturn;
static inline void raise_partial() Noreturn;
static inline void raise_bad_utf() Noreturn;
static inline void raise_bad_utf_offset() Noreturn;
static inline void raise_match_limit() Noreturn;
static inline void raise_depth_limit() Noreturn;
static inline void raise_bad_pattern(int code, size_t pos) Noreturn;
...
static inline void raise_bad_pattern(int code, size_t pos)
{
  ...
  pcre2_get_error_message(code, String_val(v_msg), 128);
...
CAMLprim value pcre2_compile_stub(value v_opt, value v_tables, value v_pat)
{
  ...
  if (v_tables != None) {
    ...
    pcre2_set_character_tables(ccontext, (chartables)Field(Field(v_tables, 0), 1));
  }

  /* Compiles the pattern */
  pcre2_code* regexp = pcre2_compile(String_val(v_pat), length, Int_val(v_opt), &error_code,
                                     &error_ofs, ccontext);
  ...
  v_rex = caml_alloc_final(4, pcre2_dealloc_regexp, 1, 1000000);

  /* Field[1]: compiled regular expression (Field[0] is finalizing
     function! See above!) */
  Field(v_rex, 1) = (value) regexp;

  /* Field[2]: match context used for regexp */
  Field(v_rex, 2) = (value) pcre2_match_context_create(NULL);

  return v_rex;
}

/* Sets a depth limit for a regular expression imperatively */
CAMLprim value pcre2_set_imp_depth_limit_stub(value v_rex, value v_lim) {
  pcre2_match_context* mcontext = (pcre2_match_context*)Field(v_rex, 2);
...
{
  return pcre2_pattern_info((pcre2_code *) Field(v_rex, 1), what, where);
}
...
#define make_info(tp, cnv, name, option) \
  CAMLprim value pcre2_##name##_stub(value v_rex) \
  { \
    tp options; \
...
make_info(unsigned long, Val_long, argoptions, ARGOPTIONS)
...
CAMLprim value pcre2_lastcodeunit_stub(value v_rex)
{
  ...
  if (lastcodetype < 0) raise_internal_error("pcre2_lastcodeunit_stub");
  else {
    /* Allocates [Some char] */
...
static inline void handle_match_error(char *loc, const int ret) Noreturn;
...
/* Executes a pattern match with runtime options, a regular expression, a
   ...
   function */
CAMLprim value pcre2_match_stub(value v_opt, value v_rex, value v_pos,
                                ...
                                value v_ovec, value v_maybe_cof)
{
  ...
  {
    const pcre2_code *code = (pcre2_code *) Field(v_rex, 1);  /* Compiled pattern */
    pcre2_match_context* mcontext = (pcre2_match_context*)Field(v_rex, 2);  /* Match context */
    const char *ocaml_subj = String_val(v_subj) + subj_start;  /* Subject string */
    ...
    /* There are callout functions */
      ...
      Begin_roots4(v_rex, v_cof, v_substrings, v_ovec);
        ...
        ret = pcre2_match(code, subj, len, pos, opt, match_data, new_mcontext);
...
{
  return pcre2_match_stub(argv[0], argv[1], argv[2], argv[3],
...
CAMLprim value pcre2_maketables_stub(value __unused v_unit)
{
  /* GC will do a full cycle every 1_000_000 table set allocations (one
     ...
     table sets) */
  const value v_res = caml_alloc_final(2, pcre2_dealloc_tables, 1, 1000000);
  Field(v_res, 1) = (value) pcre2_maketables(NULL);
  return v_res;
}
...
CAMLprim value pcre2_substring_number_from_name_stub(value v_rex, value v_name)
{
  const int ret = pcre2_substring_number_from_name((pcre2_code *) Field(v_rex, 1),
                                        String_val(v_name));
...
CAMLprim value pcre2_config_link_size_stub(value __unused v_unit)
{ return Val_int(pcre2_config_int(PCRE2_CONFIG_LINKSIZE)); }

/* Returns boolean indicating use of stack recursion */
CAMLprim value pcre2_config_stackrecurse_stub(value __unused v_unit)
{ return Val_bool(pcre2_config_int(PCRE2_CONFIG_STACKRECURSE)); }

/* Returns default limit for calls to internal matching function */
CAMLprim value pcre2_config_match_limit_stub(value __unused v_unit)
{ return Val_long(pcre2_config_long(PCRE2_CONFIG_MATCHLIMIT)); }

/* Returns default limit for depth of nested backtracking  */
CAMLprim value pcre2_config_depth_limit_stub(value __unused v_unit)
{ return Val_long(pcre2_config_long(PCRE2_CONFIG_DEPTHLIMIT)); }
